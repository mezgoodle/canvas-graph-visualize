...
namespace matrix
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < n; i++)
                for (int j = 0; j < n; j++)
                    matrix_oriented[i,j] = Math.Floor((Convert.ToDouble(rand.Next(200))/100) * (1.0 - n3 * 0.01 - n4 * 0.01 - 0.3));

            for (int i = 0; i < n; i++)
                for (int j = 0; j < n; j++)
                    if (matrix_oriented[i,j] == 1)
                    {
                        matrix_non_oriented[i,j] = matrix_oriented[i,j];
                        matrix_non_oriented[j,i] = matrix_oriented[i,j];   
                    }
